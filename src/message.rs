...
//! the workstation at 44, the encrypted session key at 52 and the negotiated
//! flags at 60; a version and the MIC follow, to 88. An `NTLMv2` NT
...
pub(crate) mod tests {
    ...
    pub(crate) fn type3(user: &str, domain: &str, response: &[u8]) -> Vec<u8> {
        type3_exchanging(user, domain, response, 0, &[])
    }

    /// The same under further flags, carrying an encrypted session key. The
    /// version and the MIC are zero; the payload begins at 88, after them.
    pub(crate) fn type3_exchanging(
        user: &str,
        domain: &str,
        response: &[u8],
        flags: u32,
